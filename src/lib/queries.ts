...
export const GET_COMUNA_BY_SLUG = `*[_type == "comuna" && slug.current == $slug][0]{
  name,
  "slug": slug.current,
  price,
  subtitle,
...
export async function fetchComuna(slug: string): Promise<Comuna | null> {
  try {
    const comuna = await client.fetch(GET_COMUNA_BY_SLUG, { slug })
    return comuna
  } catch (error) {
